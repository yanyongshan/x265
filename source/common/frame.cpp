...
bool Frame::create(x265_param *param, float *quantOffsets) {
    ...
    if (param->rc.aqMode == X265_AQ_EDGE || (param->rc.zonefileCount && param->rc.aqMode != 0)) {
        ...
        uint32_t m_lumaMarginX =
                param->maxCUSize + 32; // search margin and 8-tap filter half-length, padded for 32-byte alignment
...
bool Frame::allocEncodeData(x265_param *param, const SPS &sps) {
    ...
    if (ok) {
        ...
        if (param->internalCsp != X265_CSP_I400) {
            memset(m_reconPic->m_picOrg[1], 0,
                   sizeof(pixel) * m_reconPic->m_strideC * (maxHeight >> m_reconPic->m_vChromaShift));
            memset(m_reconPic->m_picOrg[2], 0,
                   sizeof(pixel) * m_reconPic->m_strideC * (maxHeight >> m_reconPic->m_vChromaShift));
